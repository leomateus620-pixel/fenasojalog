...
import ScooterReservationCard from './ScooterReservationCard';
import ScooterReservationDialog from './ScooterReservationDialog';
...
export default function ScooterReservationsTab() {
  const { reservations, setStatus } = useScooterReservations();
  ...
  const [editing, setEditing] = useState<ScooterReservation | null>(null);
  const [cancelTarget, setCancelTarget] = useState<ScooterReservation | null>(null);

  const scooterMap = useMemo(() => {
    const m = new Map<string, any>();
    scooters.forEach((s: any) => m.set(s.id, s));
    return m;
  }, [scooters]);
  ...
  const filtered = useMemo(() => {
    ...
    return reservations.filter((r) => {
      ...
      if (!s) return true;
      const sc = scooterMap.get(r.scooter_id);
      const responsavel = r.responsavel_user_id ? memberMap.get(r.responsavel_user_id) : null;
      const haystack = [sc?.codigo, sc?.nome, r.nome_externo, r.empresa_slug, r.comissao, responsavel?.nome_exibicao]
        .filter(Boolean).join(' ').toLowerCase();
      return haystack.includes(s);
    });
  }, [reservations, period, search, scooterMap, memberMap]);
  ...
  const handleStart = async (r: ScooterReservation) => {
    try {
      await pickup.mutateAsync({
        ...
        telefone_externo: r.telefone_externo,
      });
        ...
        <div className="relative flex flex-col sm:flex-row gap-3 items-stretch sm:items-center">
          <div className="relative flex-1 max-w-md">
            ...
            <Input
              ...
              placeholder="Buscar por patinete, responsável..."
          ...
          <div className="col-span-full text-center py-16 text-muted-foreground">
            ...
            <p className="text-xs text-muted-foreground/70 mt-1">
              {reservations.length === 0
                ? 'Crie reservas antecipadas de patinetes para autorizados, parceiros ou equipe interna'
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              A reserva será marcada como cancelada e o patinete ficará livre nesse período. Essa ação não exclui o registro.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
