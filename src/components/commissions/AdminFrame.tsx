    ...
    <div className="min-h-screen bg-background grain-texture">
      ...
      <header className="sticky top-0 z-30 border-b border-border/60 bg-card/80 backdrop-blur-2xl">
        <div className="mx-auto flex max-w-7xl flex-col gap-3 px-4 py-3 md:flex-row md:items-center md:justify-between md:px-6">
          <Link to="/admin" className="flex min-w-0 items-center gap-3 focus-ring rounded-2xl">
            <img src={logo} alt="Fenasoja" className="h-10 w-10 rounded-2xl bg-primary/10 object-contain p-1" />
          ...
          <nav className="flex flex-wrap items-center gap-2" aria-label="Admin">
            {navItems.map((item) => (
              <NavLink
                ...
                className={({ isActive }) =>
                  cn(
                    'rounded-2xl px-3 py-2 text-sm font-semibold transition focus-ring',
            ...
            <button
              type="button"
              onClick={signOut}
              className="rounded-2xl px-3 py-2 text-sm font-semibold text-muted-foreground transition hover:bg-red-500/10 hover:text-red-600 focus-ring"
            >
      ...
      <main id="main-content" className="mx-auto flex w-full max-w-7xl flex-col gap-5 px-4 py-6 md:px-6">
        <section className="premium-surface gold-accent rounded-2xl p-5 md:p-6">
