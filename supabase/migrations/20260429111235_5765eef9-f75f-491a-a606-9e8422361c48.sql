-- 1) Função segura para publicar localização do motorista
...
BEGIN
  ...
  IF v_transport.motorista_user_id IS NULL OR v_transport.motorista_user_id <> v_user THEN
    RAISE EXCEPTION 'Apenas o motorista designado pode publicar localização';
  END IF;
...
-- 2) Limpeza de linhas fantasmas (viagens não-ativas)
...
-- 3) Backfill de coordenadas faltantes
